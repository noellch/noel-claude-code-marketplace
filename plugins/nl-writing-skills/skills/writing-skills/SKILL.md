...
- **Pruning** — **single source of truth** (one authoritative place per meaning); **relevance** (does the line still bear on the job?); **no-op** test (does it change behaviour vs. the default? weak *be thorough* → stronger *relentless*).
...
| A line the model already obeys by default | **no-op** | Delete it, or replace a weak leading word with a stronger one |
